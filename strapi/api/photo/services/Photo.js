...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Photo.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Photo.query(buildQuery({ model: Photo, filters }))
  ...
  count: (params) => {
    ...
    return Photo.query(buildQuery({ model: Photo, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Photo.forge(params).save(data);
